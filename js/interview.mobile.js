function ushahidiDate(date)
{
	...
	if(incident_hour > 12)
	{
		incident_hour = incident_hour - 12;
	}
	if(incident_hour == 0)
	{
		incident_hour = 12;
...
(function(context)
{
	...
	{
		// get the  position
		geo_position_js.getCurrentPosition(context._newInterview, geo_error_callback,
		{
	...
	context._newInterview = function(position)
	{
		...
		// incident_title
		context.addAnswer(interview, 'incident_title', $("#incident_title").val().toString());
		// incident_description
		context.addAnswer(interview, 'incident_description', $("#incident_description").val().toString());
		// incident_date
		context.addAnswer(interview, 'incident_date', ushahidi_date['incident_date']);
		// incident_hour
		context.addAnswer(interview, 'incident_hour', ushahidi_date['incident_hour']);
		// incident_minute
		context.addAnswer(interview, 'incident_minute', ushahidi_date['incident_minute']);
		// incident_ampm
		context.addAnswer(interview, 'incident_ampm', ushahidi_date['incident_ampm']);
		// incident_category
		context.addAnswer(interview, 'incident_category', $("#incident_category").val().toString());
		// latitude
		context.addAnswer(interview, 'latitude', latitude);
		// longitude
		context.addAnswer(interview, 'longitude', longitude);
		// location_name
		context.addAnswer(interview, 'location_name', $("#location_name").val().toString());
		// incident_photo
